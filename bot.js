...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.find("name", "INFINITY")
...
client.on('guildMemberAdd', member => {
    let channel = member.guild.channels.find('chat', 'welcomee');
    ...
    var channel =member.guild.channels.find('chat', 'welcome');
    });
...
if(message.content.startsWith("مسح")) {            //Codes Development .
...
client.on('message', message => {
    ...
    if (message.content.startsWith("cc1")) {
